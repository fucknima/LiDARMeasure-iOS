...
## v0.2.0 整改内容（已完成，CI Green）

将「Saliency 伪检测」彻底重构为「真实 CoreML 目标检测」流水线：

- [x] CoreML YOLOv8n（COCO 80 类，raw 输出 [1,84,8400]，FP16）随 App 打包（`LiDARMeasure/ML/ObjectDetector.mlpackage`）
- [x] macOS CI 导出模型 workflow（`export-model.yml` + `Scripts/export_model.py`）
- [x] XcodeGen preBuildScript 用 coremlcompiler 把模型编译进 app bundle（mlmodelc 验证通过）
- [x] ObjectDetector：`MLModel + VNCoreMLModel + VNCoreMLRequest`（.scaleFill）+ YOLODecoder（decode + NMS）
- [x] 目标跨帧跟踪（IoU + 类别 + 中心距离，保持稳定 ID）
- [x] 实例分割按 IoU 匹配 YOLO 检测框（FinalMask = 检测框 ∩ 实例掩码），禁止整张前景 mask
- [x] 深度直方图 dominant peak 动态前景带 + validDepthRatio 统计
- [x] PointCloudBuilder 重构：只扫目标 ROI、内参按深度图分辨率缩放、mask 采样
- [x] 重力对齐 OBB（PCA2D 闭式解）+ MAD 预过滤 + 1%~99% percentile 裁剪
...
- [x] COCO 80 类中文映射
- [x] 单元测试 34 个全过（OBB/PCA/tracker/mapping/stabilizer/decoder 等）
- [x] CI：模型存在性检查 + mlmodelc 编译进 app 检查 + unsigned IPA artifact
- [x] 合并 master、tag v0.2.0、Release 发布
